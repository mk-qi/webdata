...
如上图 
 ...
 1. Docker容器的启动 基本上就是一个应用进程的启动,所以单位是秒级的.虚拟机则还有操作系统额外起关时间
 2. 相对VM,容器进程少了VM的CPU指令和IO等额外转换的开销,所以其系统资源的利用率会高一些,几乎同等物理机.
 3. docker 部署和维护优势 
	一个是开启和关闭的时时间短,另一个境像管理,在起容器实例的时候不需要复制很多无关的文件,还有一个是能过dockfile和可以进行工作流的管理,一次构建,可以在任何地方运行,所以这个可以很大程序的降低应用部速度和维护成本.
 4. centos6.5已上的版本已经可以非常好支持docker了,基于docker本身的灵活性,基本上融合进当前的运行架构中,不需要做太多的调整.
...
关于docker的网络互通问题,分解下来,主要是下面几个问题,以下几个问题都有各种解方法,展开来讲将又是一个很大的话题,所以这里不展开太多,仅以docker 默认的解决方法做个说明
