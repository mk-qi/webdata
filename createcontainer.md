...
如上图 
 ...
 1. Docker容器的启动 基本上就是一个应用的启动,基本上秒级的，因为抛开了操作系统启动的时间,这相比传统的虚拟机方式要快得多.
 2. 应用少了虚拟操作系统的额外开销,所以对系统资源的利用率基本上就是物理机同等,一个容器基本上就是一个或多个进程,所以一台主机上可以同时运行很多个容器.
 3. 部署和维护优势 
	 通过dockfile 或是其它方式做一次构建,可以在任何地方运行多次,所以可以大的降低部署速度和维护成本.

 4. 可以非常好的融合进当前的运行架构,整个架构不需要做太多的调整.
